...
class FaceExtractionModel:
        ...
        if initial_faces_count == 0:
            ...
            for i in extractions_org:
                if self.cv_dnn_detect_faces(i, second_conf, display=False)[4] > 0:
                    ...
                    ## Print extraction_index, len(extractions), len(extractions_org)                    
    ...
    def extract_faces(self):
        ...
        for filename in os.listdir(self.input_directory):
            ...
            if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".jpeg"):
                ...
                # Use image as input
                initial_count, final_count = self.two_pass_face_detection(image, 0.121, 0, image_count, filename)
    ...
    def one_pass_extract_faces(self):
        ...
        for filename in os.listdir(self.input_directory):
            ...
            if filename.endswith(".jpg") or filename.endswith(".png") or filename.endswith(".jpeg"):
                ...
                # Use image as input
                output_image, results, faces, faces_org, faces_count = self.cv_dnn_detect_faces(image, 0.137, display = False)
                ...
                for i in faces:
                    ...
                    output_directory = self.output_directory
                    os.makedirs(output_directory, exist_ok=True)
                    output_filename = f"{filename}-face{face_index}.jpg"
                    output_path = os.path.join(output_directory, output_filename)
                    cv2.imwrite(output_path, i)
                    face_index += 1

                print(f"Extracted {faces_count} faces from {filename}.")
